...
CREATE PROCEDURE [dbo].[TableRestaurant_GetAll]
AS
SELECT * FROM TableRestaurant
go

CREATE PROCEDURE [dbo].[TableRestaurant_InsertUpdateDelete]
@ID int output, -- Biến ID tự tăng, khi thêm xong phải lấy ra
@Name nvarchar(1000),
@Status int,
@Capacity int,
...
IF @Action = 0 -- Nếu Action = 0, thêm dữ liệu
BEGIN
INSERT INTO [dbo].[TableRestaurant] ([Name],[Status],[Capacity])
VALUES (@Name, @Status,@Capacity)
SET @ID = @@identity -- Thiết lập ID tự tăng
END
ELSE IF @Action = 1 -- Nếu Action = 1, cập nhật dữ liệu
BEGIN
UPDATE [dbo].[TableRestaurant]
SET [Name] = @Name,[Status] =@Status,[Capacity]=@Capacity
WHERE [ID] = @ID
END
ELSE IF @Action = 2 -- Nếu Action = 2, xóa dữ liệu
BEGIN
DELETE FROM [dbo].[TableRestaurant] WHERE [ID] = @ID
END
GO